...
Strengths:
1. RBFs are able to perform optimization over an n-dimensional space using a relatively simple formula, making them computationally inexpensive while still having a lot of power. 
...
4. The hyperparameter $\gamma$ and the cost parameter allow us to be more flexible about model specificity. 

Weaknesses: 
1. RBFs are non probablistic, meaning points are only classified as being in one class or the other. We cannot adjust the probability cutoff for classifying points like we might in logistic regression. 
...
validation_data_frame <- cross_validate(5, c(0.1, 1, 10, 100, 1000),
 c(0.01, 0.1, 1, 10, 100), data2, seed = 2024)
...
# Create a confusion matrix to evaluate the SVM model
confusionMatrix(predict(svmfit_best, test_data2), test_data2$class) |> simpler_cm()
```
...
# Running this code will take awhile to run, to speed it up we will only use 2 folds
#validation_data_frame_poly <- cross_poly(2, degrees, costs,
# gammas, data2, seed = 2024)

# Print the top 5 best degree, cost, and gamma values
#print(validation_data_frame_poly[order(-validation_data_frame_poly$accuracy),
# ][1:5, ])
```
...
plot(grid$x, grid$y, col = boundary_colors[predicted_labels_poly], pch = ".", cex = 3.5,
 xlab = "x", ylab = "y")
...
confusionMatrix(as.factor(predicted_probabilities),
 as.factor(data2$class)) |> simpler_cm()

# Plot the decision boundary
predicted_labels_logit <- predicted_probabilities > 0.5
plot(grid$x, grid$y, col = boundary_colors[predicted_labels_logit + 1], pch = ".",
 cex = 3.5, xlab = "x", ylab = "y")
```
...
knn_model <- knn(train = data2[, c("x", "y")], test = data2[, c("x", "y")],
 cl = data2$class, k = k)
...
# Create a confusion matrix to evaluate the model
confusionMatrix(as.factor(knn_model), data2$class) |> simpler_cm()

# Create the grid for the decision boundary
predicted_labels_knn <- knn(train = data2[, c("x", "y")], test = grid,
 cl = data2$class, k = k)
```
```{r, echo=FALSE}
# Convert knn_model to numeric
...
```{r}
# Plot the decision boundary
plot(grid$x, grid$y, col = boundary_colors[predicted_labels_knn + 1], pch = ".",
 cex = 3.5, xlab = "x", ylab = "y")
...
# Create a confusion matrix to evaluate the model
confusionMatrix(as.factor(knn_model_test), test_data2$class) |> simpler_cm()
```
...
# Plot with base R
plot(mnist_27$train$x_1, mnist_27$train$x_2, col = point_colors[mnist_27$train$y], pch = 19, xlab = "Proportion of dark pixels in the upper-left quadrant", ylab = "Proportion of dark pixels in the lower-right quadrant")

# Add a legend
legend("topright", legend = levels(as.factor(mnist_27$train$y)), col = point_colors, pch = 19)
```
...
# Create a confusion matrix to evaluate the SVM model
confusionMatrix(predict(svmfit_linear_mnist, mnist_27$train), mnist_27$train$y) |> simpler_cm()
...
# Create a confusion matrix to evaluate the SVM model
confusionMatrix(predict(svmfit_rbf_mnist, mnist_27$train), mnist_27$train$y) |> simpler_cm()
...
# Create a confusion matrix to evaluate the SVM model
confusionMatrix(predict(svmfit_best_mnist, mnist_27$train), mnist_27$train$y) |> simpler_cm()
...
```{r, echo=FALSE}
# Create a confusion matrix to evaluate the SVM model
confusionMatrix(predict(svmfit_best_mnist, mnist_27$test), mnist_27$test$y) |> simpler_cm()
```
...
# Create a confusion matrix to evaluate the model
confusionMatrix(as.factor(knn_model_mnist), training_matrix$class) |> simpler_cm()
...
# Create a confusion matrix to evaluate the model
confusionMatrix(as.factor(knn_model_test_mnist), mnist_27_test$class) |> simpler_cm()
