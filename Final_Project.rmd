...
&\text{if } K(\boldsymbol{\beta}, \xb_i) - c > 0 \rightarrow \hat{y_i} = 1\\
&\text{if } K(\boldsymbol{\beta}, \xb_i) - c < 0 \rightarrow \hat{y_i} = -1
\end{align*}
...
2.- $K$, the matrix composed of all pairwise kernels ($k_{ij} = K( \xb_i , \xb_j )$), is positive semi-definite. This is satisfied by the Radial Basis Kernel.

### Model Parameters
The RBF model has two tunne-able parameters: cost and gamma. 

1.- The cost parameter ($C_\xi$) is the cost of missclassifying a data point (Soft Margin case only). A higher $C_\xi$ will lead to a more complex model that will try to classify all data points correctly. It trades off missclassification of training examples against increasing the margin. Larger values of cost will lead to a smaller margin. In this way $C_\xi$ behaves as a regularization parameter. 
...
We can observe that the decision boundary doesn't capture the spiraly-ness of the data very well. This is in part since we used the default $C_\xi$ and $\gamma$.

### Cross Validating RBF
We can use cross-validation to find the best values for $C_\xi$ and $\gamma$. Fortunately, the `svm` function has built in cross-validation. Setting the `cross` parameter to the number of folds, in this case 5. However, the `svm` function does not *find* the best set of parameters, it only calculates statistics for a given set of parameters. The following function performs the full cross-validation for a RBF model:
...
# Running this takes forever so only use 2 folds :(
