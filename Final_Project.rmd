...
```{python, echo=F, cache=T, out.width="70%"}
import matplotlib
...
Once again, we get weirness as our Kernel degree gets higher.
...
cross_knn <- function(folds, ks, dataset, seed) {
  ...
  for (k in ks) {
    ...
    for (fold in seq_along(folds_obj)) {
      ...
      knn_model <- knn3(y ~ ., train = train_data, test = test_data, cl = train_labels, k = k)
...
knn_model <- knn3(train = data2[, c("x", "y")], test = data2[, c("x", "y")], cl = data2$class, k = k)
...
predicted_labels_knn <- knn3(train = data2[, c("x", "y")], test = grid, cl = data2$class, k = k)
```
```{r, echo=FALSE}
# Convert knn_model to numeric
...
knn_model_test <- knn3(train = data2[, c("x", "y")], test = test_data2[, c("x", "y")], cl = data2$class, k = k)
...
# Run the cross validation function
validation_data_frame_knn_mnist <- cross_knn(10, ks, training_matrix, seed = 5000)
...
# Use a KNN model to classify the data
knn_model_mnist <- knn3(train = training_matrix[, c("x", "y")], test = training_matrix[, c("x", "y")], cl = training_matrix$class, k = k)
...
predicted_labels_knn_mnist <- knn3(train = training_matrix[, c("x", "y")], test = grid, cl = training_matrix$class, k = k)
...
# Create a confusion matrix to evaluate the KNN model
knn_model_test_mnist <- knn3(train = training_matrix[, c("x", "y")], test = mnist_27_test[, c("x", "y")], cl = training_matrix$class, k = k)
